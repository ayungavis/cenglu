...
|-----------|------|-------|
| Logger creation | ~0.001ms | Minimal setup |
| Simple log (no context) | ~0.003ms | Level check + format |
| Log with context | ~0.005ms | Including merge |
| Child logger creation | ~0.002ms | Shared resources |
| Error serialization | ~0.010ms | Stack trace parsing |
